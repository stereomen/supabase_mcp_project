...
async function getLocationsForData(supabase, column) {
  ...
  if (error) {
    console.error(`'${column}' 컬럼에 대한 위치 정보 조회 중 오류 발생:`, error);
    throw new Error(`'${column}'에 대한 데이터 조회에 실패했습니다.`);
  }
  
  // GROUP BY 효과를 위해 중복 제거 (station_id, name, latitude, longitude 조합)
  const uniqueLocations = [];
  const seenStations = new Set();
  
  (data || []).forEach(location => {
    const key = `${location.station_id}-${location.name}-${location.latitude}-${location.longitude}`;
...
Deno.serve(async (_req)=>{
  ...
  try {
    ...
    console.log("분석 완료. 성공 메시지를 전송합니다.");
