#!/usr/bin/env python3
"""
tidedata.csv의 지역들을 중기예보 지역과 매칭하는 스크립트
지역명 기반으로 가장 유사한 중기예보 지역을 찾아서 매칭
"""
...
def find_best_match(location_name, forecast_regions):
    ...
    for key, target in name_mappings.items():
        if key in location_name:
            if target in forecast_regions:
                return forecast_regions[target]
    ...
    if best_match is None or best_score < 0.3:
        # 위도/경도 기반으로 대략적 지역 분류 후 기본값 설정
        # 이 부분은 tidedata의 위도/경도 정보를 활용할 수 있음
...
def main():
    ...
    with open('tidedata.csv', 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            ...
            # 가장 적합한 중기예보 지역 찾기
            match = find_best_match(location_name, forecast_regions)
            ...
            print(f"{location_name} -> {match['REG_NAME']} ({match['REG_ID']})")
    
    # 새로운 CSV 파일로 저장
    with open('tidedata_with_weather_regions.csv', 'w', encoding='utf-8', newline='') as f:
        fieldnames = ['Code', 'Name', 'Latitude', 'Longitude', 'REG_ID', 'REG_SP', 'REG_NAME']
        writer = csv.DictWriter(f, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(matched_data)
    
    print(f"\n매칭 완료! 결과가 tidedata_with_weather_regions.csv에 저장되었습니다.")
